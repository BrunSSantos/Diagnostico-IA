...
import { excluirUsuario } from '@/app/actions/users/excluirUsuario';
...
const ExpandingList: React.FC<ExpandingListProps> = ({ items }) => {
  ...
  const handleClick = async(itemId: number, cargoN: string) => {
    ...
    if(cargoSelecionado !== undefined){
      ...
      if(cargoN === "Médico" || cargoN === "Enfermeiro"){
       const mensagem = await alterarProfSaude(itemId, expandedItemFields.nomeUsuario, expandedItemFields.email,
       expandedItemFields.registro, cargoNum);
       setMessage(mensagem)
       console.log("Profissional da saude alterado");
      }else{
        const mensagem = await alterarAdmin(itemId, expandedItemFields.nomeUsuario, expandedItemFields.email, cargoNum);
  ...
  const handleClickDelete = async(itemId: number) =>{
     ...
     if (itemId !== null){
          ...
          const mensagem = await excluirUsuario(itemId);
          setMessage(mensagem)
          setExcluido(true)
          router.refresh();
     }else{
      ...
      {items.map((item) => (
        ...
        >
          ...
          <div className="flex space-x-2 mt-2">
            ...
            <button className="flex items-center pl-2 pr-4 rounded cursor-pointer fixed-button" onClick={()=> handleClickDelete(item.id)}>
