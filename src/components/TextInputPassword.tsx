...
export function InputPasswordSimples(props: InputSimplesProps) {
    const { color, size, label, value, onChange} = props;
    return (
      <div className="w-72">
        <Input  type="password" label={label} color={color} size={size} crossOrigin={undefined} value={value} onChange={(e) => onChange(e.target.value)}/>
      </div>
    );
  }
